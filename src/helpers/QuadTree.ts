...
class Quad {
  ...
  subdivide() {
    ...
    this.tree.push(
      new Quad(this.x + halfWidth, this.y + halfHeight, halfWidth, halfHeight),
    )
  ...
  intersects(x: number, y: number, width: number, height: number): boolean {
    ...
    if (
      this.inQuad(bTopLeft) ||
      this.inQuad(bTopRight) ||
      this.inQuad(bBottomLeft) ||
      this.inQuad(bBottomRight)
    ) {
      return true
    }
    ...
    for (const quad of tree) {
      ...
      if (quad.intersects(x, y, width, height)) {
        ...
        if (quad.tree.length > 0) {
          found.push(
            ...this.recursiveFindInArea(quad.tree, x, y, width, height),
          )
        }
      }
    }
